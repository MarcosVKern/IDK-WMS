...
class Cargo_View:
    ...
    def _setup_ui(self):
        ...
        self.combo_cargo = ttk.Combobox(
            frame_form, textvariable=self.var_cargo, width=50, state="readonly"
        )
    ...
    def get_cargo_data(self, cargo_existente=None):
        try:
            return {"cargo": self.var_cargo.get()}
        except Exception as e:
            Notificacao.erro(
                "Erro", f"Erro ao obter dados do cargo: {e}", parent=self.root
            )
            return None
    ...
    def _limpar_campos(self):
        for val in [self.var_id, self.var_cargo]:
            val.set("")
