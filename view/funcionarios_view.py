...
class Funcionario_View:
    ...
    def _setup_ui(self):
        tk.Label(
            self.root,
            text="GERENCIAMENTO DE FUNCIONARIOS",
            font=("Arial", 16, "bold"),
            pady=10,
        ...
        tk.Entry(frame_form, textvariable=self.var_id, state="readonly", width=15).grid(
        ...
        tk.Entry(frame_form, textvariable=self.var_nome, width=20).grid(
            row=3, column=0, pady=5, sticky="ew", padx=(0, 5)
        )

        tk.Label(frame_form, text="Cargo:", bg=Cores_Padrao.COR_FUNDO).grid(
            row=2, column=1, sticky="w", pady=5, padx=(5, 0)
        )
        self.combo_cargo = ttk.Combobox(
            frame_form, textvariable=self.var_cargo, width=17, state="readonly"
        )
        self.combo_cargo.grid(row=3, column=1, pady=5, sticky="ew", padx=(5, 0))

        tk.Label(frame_form, text="Email:", bg=Cores_Padrao.COR_FUNDO).grid(
            row=4, column=0, sticky="w", pady=5
        )
        tk.Entry(frame_form, textvariable=self.var_email, width=20).grid(
            row=5, column=0, pady=5, sticky="ew", padx=(0, 5)
        )

        tk.Label(frame_form, text="Situação:", bg=Cores_Padrao.COR_FUNDO).grid(
            row=4, column=1, sticky="w", pady=5, padx=(5, 0)
        )
        tk.Entry(
            frame_form, textvariable=self.var_situacao, state="readonly", width=20
        ).grid(row=5, column=1, pady=5, sticky="ew", padx=(5, 0))
        ...
        tk.Label(frame_endereco, text="Bairro:", bg=Cores_Padrao.COR_FUNDO).grid(
            row=2, column=0, sticky="w", pady=5
        )
        tk.Entry(frame_endereco, textvariable=self.var_bairro, width=20).grid(
            row=3, column=0, pady=5, sticky="ew", padx=(0, 5)
        )

        tk.Label(frame_endereco, text="Cidade:", bg=Cores_Padrao.COR_FUNDO).grid(
            row=2, column=1, sticky="w", pady=5, padx=(5, 0)
        )
        tk.Entry(frame_endereco, textvariable=self.var_cidade, width=20).grid(
            row=3, column=1, pady=5, sticky="ew", padx=(5, 0)
        )

        tk.Label(frame_endereco, text="UF:", bg=Cores_Padrao.COR_FUNDO).grid(
            row=4, column=0, sticky="w", pady=5
        )
        tk.Entry(frame_endereco, textvariable=self.var_uf, width=20).grid(
            row=5, column=0, pady=5, sticky="ew", padx=(0, 5)
        )

        tk.Label(frame_endereco, text="País:", bg=Cores_Padrao.COR_FUNDO).grid(
            row=4, column=1, sticky="w", pady=5, padx=(5, 0)
        )
        tk.Entry(frame_endereco, textvariable=self.var_pais, width=20).grid(
            row=5, column=1, pady=5, sticky="ew", padx=(5, 0)
        )
    ...
    def get_funcionario_data(self, funcionario_existente=None):
        try:
            ...
            Notificacao.erro(
                "Erro", f"Erro ao obter dados do funcionário: {e}", parent=self.root
            )
            return None
    ...
    def _acao_listar(self):
        if self.controller:
            self.controller.list_funcionarios()
    ...
    def _buscar_endereco_cep(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                Notificacao.sucesso(
                    "Sucesso", "Endereço preenchido com sucesso!", parent=self.root
                )
            else:
                Notificacao.erro(
                    "Erro", "Erro ao conectar com o serviço de CEP!", parent=self.root
                )
        except Exception as e:
            Notificacao.erro(
                "Erro", f"Erro ao buscar endereço: {str(e)}", parent=self.root
            )
