...
class Produto_View():
    ...
    def _setup_ui(self):
        tk.Label(self.root, text="GERENCIAMENTO DE PRODUTOS", font=("Arial", 16, "bold"), pady=10).pack()

        frame_form = tk.LabelFrame(self.root, text="Detalhes do Produto", padx=10, pady=10)
        ...
        tk.Label(frame_form, text="Descrição:").grid(row=4, column=0, sticky="w")
        tk.Entry(frame_form, textvariable=self.var_descricao, width=30).grid(row=5, column=0, pady=5)

        tk.Label(frame_form, text="URL da Imagem:").grid(row=6, column=0, sticky="w")
