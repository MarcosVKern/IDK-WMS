...
class Armazem_View:
    ...
    def _setup_ui(self):
        tk.Label(
            self.root,
            text="GERENCIAMENTO DE ARMAZÉNS",
            font=("Arial", 16, "bold"),
            pady=10,
    ...
    def get_armazem_data(self, armazem_existente=None):
        try:
            ...
            Notificacao.erro(
                "Erro", f"Erro ao obter dados do armazém: {e}", parent=self.root
            )
            return None
    ...
    def show_armazens(self, lista):
        ...
        for idx, a in enumerate(lista):
            ...
            self.tree.insert(
                "",
                "end",
                values=(a._id, a._cep, a._bairro, a._cidade, a._uf, a._pais, a._nome),
                tags=(tag,),
            )
    ...
    def _acao_deletar(self):
        if Notificacao.confirmacao(
            "Confirmação",
            "Tem certeza que deseja deletar este armazém?",
            parent=self.root,
        ):
