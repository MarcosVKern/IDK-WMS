...
class UnidadeArmazenamento_View:
    ...
    def _setup_ui(self):
        tk.Label(
            self.root,
            text="GERENCIAMENTO DE UNIDADES DE ARMAZENAMENTO",
            font=("Arial", 16, "bold"),
            pady=10,
        ...
        frame_form = tk.LabelFrame(
            self.root,
            text="Detalhes da Unidade de Armazenamento",
            padx=10,
            pady=10,
            bg=Cores_Padrao.COR_FUNDO,
        )
        ...
        tk.Label(frame_form, text="Unidade:", bg=Cores_Padrao.COR_FUNDO).grid(
        ...
        tk.Label(frame_form, text="Armazém:", bg=Cores_Padrao.COR_FUNDO).grid(
            row=4, column=0, sticky="w"
        )
        self.combobox_armazem = ttk.Combobox(
            frame_form, textvariable=self.var_armazem, width=27, state="readonly"
        )
    ...
    def get_unidade_armazenamento_data(self, unidade_existente=None):
        try:
            ...
            return {"unidade": self.var_unidade.get(), "armazem": armazem_id}
        except Exception as e:
            Notificacao.erro(
                "Erro",
                f"Erro ao obter dados da unidade de armazenamento: {e}",
                parent=self.root,
            )
            return None
    ...
    def show_unidades_armazenamento(self, lista):
        ...
        for idx, u in enumerate(lista):
            ...
            self.tree.insert(
                "", "end", values=(u._id, u._unidade, u._armazem), tags=(tag,)
            )
    ...
    def _acao_deletar(self):
        if Notificacao.confirmacao(
            "Confirmação",
            "Tem certeza que deseja deletar esta unidade de armazenamento?",
            parent=self.root,
        ):
