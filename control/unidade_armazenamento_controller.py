...
class UnidadeArmazenamento_Controller:
    ...
    def add_unidade_armazenamento(self):
        try:
            ...
            self.view.show_message(f"Unidade de armazenamento '{unidade_salva._unidade}' adicionada com id {unidade_salva._id}!")
        except Exception as e:
            self.view.show_error(f"Erro ao adicionar unidade de armazenamento: {str(e)}")

    def delete_unidade_armazenamento(self):
        try:
            id_unidade = self.view.get_id()
            if self.dao.delete(id_unidade):
                self.view.show_message(f"Unidade de armazenamento com id {id_unidade} deletada com sucesso!")
            else:
                self.view.show_message(f"Unidade de armazenamento com id {id_unidade} não encontrada!")
        except Exception as e:
    ...
    def get_unidade_armazenamento(self):
        try:
            ...
            if not unidade:
                self.view.show_error(f"Unidade de armazenamento com id {id_unidade} não encontrada!")
                return
