...
class MovimentoEstoque_Controller:
    ...
    def add_movimento(self):
        try:
            if self.funcionario_logado and self.funcionario_logado._cargo not in [
                1,
                2,
                3,
            ]:
                self.view.show_error(
                    "Você não tem permissão para criar movimentos de estoque!"
                )
                return
            ...
            if "saída" in tipo_nome or "saida" in tipo_nome:
                if not dados["origem"]:
                    self.view.show_error(
                        "Para movimentos de Saída, Unidade de Origem é obrigatória!"
                    )
                    return
            elif "entrada" in tipo_nome:
                if not dados["destino"]:
                    self.view.show_error(
                        "Para movimentos de Entrada, Unidade de Destino é obrigatória!"
                    )
                    return
            elif "interno" in tipo_nome:
                if not dados["origem"]:
                    self.view.show_error(
                        "Para movimentos Internos, Unidade de Origem é obrigatória!"
                    )
                    return
                if not dados["destino"]:
                    self.view.show_error(
                        "Para movimentos Internos, Unidade de Destino é obrigatória!"
                    )
                    return
            ...
            produtos_selecionados = self._abrir_selecao_produtos(
                tipo_nome, dados.get("origem"), dados.get("destino")
            )
            if produtos_selecionados is None:
                self.view.show_message(
                    "Criação de movimento cancelada (sem seleção de produtos)."
                )
                return
            ...
            for item in produtos_selecionados:
                produto_mov = ProdutoMovimento(
                    item["quantidade"], movimento_salvo._id_movimento, item["produto"]
                )
                self.produto_movimento_dao.save(produto_mov)

            self.view.show_message(
                f"Movimento criado com sucesso com {len(produtos_selecionados)} produtos!"
            )
    ...
    def update_movimento(self, id_movimento=None):
        ...
        try:
            ...
            tipo_movimento = self.tipo_movimento_dao.get_by_id(
                movimento_existente._tipoMovimento
            )
            tipo_nome = tipo_movimento._tipo.lower().strip()
            status_atual = movimento_existente._status

            if (
                "saída" in tipo_nome or "saida" in tipo_nome or "interno" in tipo_nome
            ) and status_atual == "Em separação":
                if (
                    self.funcionario_logado
                    and self.funcionario_logado._id != movimento_existente._responsavel
                ):
                    self.view.show_error(
                        "Apenas o funcionário responsável pode confirmar o despacho!"
                    )
                    return

            # Determinar próximo status
            proximo_status = self._obter_proximo_status(
                tipo_movimento._id, status_atual
            )
            ...
            if proximo_status == "Despachado" and (
                "saída" in tipo_nome or "saida" in tipo_nome or "interno" in tipo_nome
            ):
            ...
            self._aplicar_logica_estoque(
                movimento_existente, tipo_movimento._id, status_atual, proximo_status
            )

            self.view.show_message(
                f"Movimento atualizado para status: {proximo_status}"
            )
    ...
    def _confirmar_produtos(self, produtos):
        ...
        if not produtos:
            return messagebox.askyesno(
                "Confirmação", "Nenhum produto neste movimento. Deseja continuar?"
            )
    ...
    def _aplicar_logica_estoque(self, movimento, tipo_id, status_anterior, status_novo):
        ...
        try:
            produtos = self.produto_movimento_dao.get_by_movimento_id(
            ...
            for produto in produtos:
                ...
                if tipo_id == 2 and status_novo == "Despachado":
                    ...
                    if estoque:
                        ...
                        if novo_valor < 0:
                            novo_valor = 0
                        self.estoque_dao.upsert(
                            id_produto, movimento._origem, novo_valor
                        )
                ...
                # Interno: Comportamento especial conforme status
                    ...
                    if status_novo == "Despachado":
                        # Remove de origem
                        estoque_origem = self.estoque_dao.get_by_id(
                            id_produto, movimento._origem
                        )
                        if estoque_origem:
                        ...
                        # Adiciona no destino
                        estoque_destino = self.estoque_dao.get_by_id(
                            id_produto, movimento._destino
                        )
                        quantidade_atual = (
                            estoque_destino._quantidade if estoque_destino else 0
                        )
                        novo_valor = quantidade_atual + quantidade
                        self.estoque_dao.upsert(
                            id_produto, movimento._destino, novo_valor
                        )
    ...
    def cancela_movimento(self, id_movimento=None):
        ...
        try:
            ...
            if self.funcionario_logado and self.funcionario_logado._cargo not in [
                ...
                self.view.show_error(
                    "Apenas supervisor ou diretor podem cancelar movimentos!"
                )
                return
            ...
            if not self.dao.pode_cancelar(id_movimento):
                self.view.show_error(
                    "Movimentos internos em status 'Efetivado' não podem ser cancelados!"
                )
                return
    ...
    def list_movimentos(self):
        try:
            ...
            pode_criar = self.funcionario_logado and self.funcionario_logado._cargo in [
                1,
                2,
                3,
            ]
    ...
    def abrir_detalhe_movimento(self, id_movimento):
        ...
        try:
            ...
            try:
                produtos_movimento = self.produto_movimento_dao.get_by_movimento_id(
                    id_movimento
                )
            except Exception as e:
            ...
            view_detalhe = MovimentoEstoque_Detalhe_View(
                movimento,
                tipo_movimento,
                unidade_origem,
                unidade_destino,
    ...
    def _abrir_selecao_produtos(self, tipo_nome, origem, destino):
        if "saída" in tipo_nome or "saida" in tipo_nome or "interno" in tipo_nome:
            ...
            for p in self.produto_dao.get_all():
                produtos_disponiveis.append(
                    {"id": p._id, "nome": p._nome, "quantidade": 0}
                )
        else:
            produtos_disponiveis = []
        ...
        if not produtos_disponiveis:
            if "entrada" not in tipo_nome:
                self.view.show_error(
                    "Não há produtos disponíveis para selecionar nessa unidade de origem"
                )
                return []

        view_produtos = MovimentoEstoqueProdutos_View(
            tipo_nome.capitalize(), origem, destino, produtos_disponiveis
        )
    ...
    def _obter_funcionarios_ativos(self):
        try:
            ...
            return [
                ...
                if f._situacao == "Ativo" and f._cargo == 4
            ]
        except:
            return []
