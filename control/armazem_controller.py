    ...
    def add_armazem(self):
        try:
            ...
            self.view.show_message(
                f"Armazém '{armazem_salvo._nome}' adicionado com id {armazem_salvo._id}!"
            )
    ...
    def update_armazem(self):
        try:
            ...
            if self.dao.update(armazem_atualizado):
                self.view.show_message(
                    f"Armazém '{armazem_atualizado._nome}' atualizado com sucesso!"
                )
        except Exception as e:
    ...
    def delete_armazem(self):
        try:
            ...
            if self.dao.delete(id_armazem):
                self.view.show_message(
                    f"Armazém com id {id_armazem} deletado com sucesso!"
                )
            else:
                self.view.show_message(f"Armazém com id {id_armazem} não encontrado!")
        except mysql.connector.IntegrityError:
            self.view.show_error(
                "Não é possível excluir este armazém pois ele possui vínculos com outros dados!"
            )
        except Exception as e:
